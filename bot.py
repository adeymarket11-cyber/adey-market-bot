...
def send_welcome(message):
    welcome_text = (
        ...
        "• እዚህ ዎ ውስጥ ጥቅም ላይ ውለዋል አዲስ እቃዎችን 🛒 ለማግኘት ወይም 📞 ለመሸጥ የሚችሉበት መድረክ ነው ኝን\n"
...
def handle_message(message):
    ...
    if message.content_type == 'photo':
        if str(chat_id) == str(ADMIN_CHAT_ID) and chat_id in admin_editing:
            ...
            btn_phone = InlineKeyboardButton("📞 0985427286", url=ADMIN_PHONE_LINK)
        ...
        if message.text == "🛒 እቃ ለመግዛት":
            ...
            markup.add(InlineKeyboardButton("🛒 ከናንተ ለመግዛት", url="https://t.me/adeymarket3"))
    ...
    # 2. ጽሁፍ ወይም ስልክ ቁጥር ሲላክ የሚሰራው ክፍል
    elif message.content_type == 'text':
        if message.text in ["🛒 እቃ ለመግዛት", "📞 እቃ ለመሸጥ"]:
            if message.text == "🛒 እቃ ለመግዛት":
                markup = InlineKeyboardMarkup()
                markup.add(InlineKeyboardButton("🛒 ከናንተ ለመግዛት", url="https://t.me/adeymarket3"))
                bot.send_message(chat_id, "🛒 እቃ ለመግዛት የሚከተለውን ሊንክ ይጠቀሙ:", reply_markup=markup)
            ...
            # ተጠቃሚው ስልክ ቁጥር ወይም ሌላ ጽሁፍ ሲልክ ወደ አድሚን በቀጥታ እንዲተላለፍ
            bot.forward_message(ADMIN_CHAT_ID, chat_id, message.id)
